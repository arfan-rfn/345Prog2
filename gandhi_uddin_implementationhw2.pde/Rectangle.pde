...
public class Rectangle {
  ...
  public boolean Are_Intersecting(Segment seg) {
    ...
    int y1 = (int) yMin;
    int y2 = (int) yMax;
    return inside(seg.getLeftPoint()) || Are_Intersecting(seg, new Segment(x1, y1, x2, y1))
        || Are_Intersecting(seg, new Segment(x1, y1, x1, y2)) || Are_Intersecting(seg, new Segment(x2, y1, x2, y2))
  ...
  public boolean Are_Intersecting(Segment l1, Segment l2) {
    String[] coordsStrs = { String.valueOf(l1.x1), String.valueOf(l1.y1), String.valueOf(l1.x2), String.valueOf(l1.y2),
        String.valueOf(l2.x1), String.valueOf(l2.y1), String.valueOf(l2.x2), String.valueOf(l2.y2) };
    Double[] cDs = new Double[8];
    for (int i = 0; i < 8; i++)
      cDs[i] = Double.parseDouble(coordsStrs[i]);
